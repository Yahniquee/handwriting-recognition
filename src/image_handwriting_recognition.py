...
#model_letters = tf.keras.models.load_model("models/cnn_emnist_letters_ep5.model")
...
fig2.suptitle("Extracted letters", fontsize=21)
letters = []
for j, letter in enumerate(words[0][:3]):
    ax[j].imshow(letter, cmap='gray', interpolation='none', aspect='equal')
    ax[j].axis('off')
"""

fig2, ax = plt.subplots(1,3)
fig2.suptitle("Input to neural network", fontsize=21)
letters = []
for j, letter in enumerate(words_EMNIST[0][:3]):
    im = letter.reshape(28, 28)
    ax[j].imshow(im, cmap='gray', interpolation='none', aspect='equal')
    ax[j].axis('off')
